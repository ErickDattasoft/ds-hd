...
export interface ResultadoAviso {
  ...
  enviado: boolean;
  motivo?: 'sin_pendientes' | 'sin_contacto_correo' | 'no_encontrada';
}
...
export class AvisarEmpresasService {
  ...
  async ejecutar(input: {
    ...
    const [config, listaVersiones] = await Promise.all([
    ...
    for (const empresaId of input.empresaIds) {
      ...
      if (!contacto?.email) {
        resultados.push({ empresaId, empresaNombre: empresa.nombre, enviado: false, motivo: 'sin_contacto_correo' });
        continue;
      }
      ...
      await this.bitacora.registrar({
        ...
        resumen: `Aviso de ${input.tipo} enviado a ${contacto.nombre} (${contacto.email})`,
      });
