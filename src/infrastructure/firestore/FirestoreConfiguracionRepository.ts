...
export class FirestoreConfiguracionRepository implements IConfiguracionRepository {
  ...
  async obtenerCotizaciones(): Promise<ConfiguracionCotizaciones> {
    const snap = await this.db.collection(COL).doc('cotizaciones').get();
    return sanearConfigCotizaciones(snap.exists ? snap.data() : null);
  }

  async guardarCotizaciones(config: ConfiguracionCotizaciones): Promise<void> {
    await this.db.collection(COL).doc('cotizaciones').set(config, { merge: true });
  }
