...
export const UsuarioMapper = {
  toDomain(uid: string, data: DocumentData): Usuario {
    return new Usuario({
      ...
      predeterminadosTicket: data.predeterminadosTicket ?? null,
      contactosSoporte: Array.isArray(data.contactosSoporte) ? data.contactosSoporte : [],
      totpSecreto: data.totpSecreto ?? null,
      totpActivo: data.totpActivo === true,
  ...
  toDocument(u: Usuario): DocumentData {
    return {
      ...
      predeterminadosTicket: u.predeterminadosTicket,
      contactosSoporte: u.contactosSoporte,
